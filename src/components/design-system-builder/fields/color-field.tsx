...
const hexToRgba = (hex) => {
    ...
    return { red: r.toString(), green: g.toString(), blue: b.toString(), alpha: (a * 255).toString() };
};
    ...
    <div className="flex space-x-2">
        <div>
            ...
            <Controller
                name={`${name}.red`}
                control={control}
                render={({ field }) => <Input {...field} placeholder="Red" onChange={(e) => onChangeRgba(e, field)} />}
            />
        </div>
        <div>
            ...
            <Controller
                name={`${name}.green`}
                control={control}
                render={({ field }) => <Input {...field} placeholder="Green" onChange={(e) => onChangeRgba(e, field)} />}
            />
        </div>
        <div>
            ...
            <Controller
                name={`${name}.blue`}
                control={control}
                render={({ field }) => <Input {...field} placeholder="Blue" onChange={(e) => onChangeRgba(e, field)} />}
            />
        </div>
        <div>
            ...
            <Controller
                name={`${name}.alpha`}
                control={control}
                render={({ field }) => <Input {...field} placeholder="Alpha" onChange={(e) => onChangeRgba(e, field)} />}
            />
        </div>
    </div>
);
...
const HexInput = ({ control, name, onChangeHex }) => (
    <div className="flex space-x-2">
        <Controller
            ...
            render={({ field }) => (
                <>
                    <ColorPicker color={field.value} onChange={(color) => onChangeHex({ target: { value: color } }, field)} />
                    <Input {...field} placeholder="Hex" onChange={(e) => onChangeHex(e, field)} />
                </>
            )}
        />
    </div>
            ...
            <FormControl>
                <div className="space-y-4">
                    <div>
                        ...
                        <HexInput control={control} name={`${name}.light.hex`} onChangeHex={handleHexChange} />
                        <RgbaInput control={control} name={`${name}.light.rgba`} onChangeRgba={handleRgbaChange} />
                    </div>

                    <div>
                        <FormLabel className="font-semibold">Dark</FormLabel>
                        <HexInput control={control} name={`${name}.dark.hex`} onChangeHex={handleHexChangeDark} />
                        <RgbaInput control={control} name={`${name}.dark.rgba`} onChangeRgba={handleRgbaChangeDark} />
                    </div>
                </div>
            </FormControl>
